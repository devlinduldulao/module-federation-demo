...
Instructions for coding agents working anywhere in this repository.

This file covers the **whole monorepo**. Each package has its own `AGENTS.md` that
overrides and extends this one. **Read the closest `AGENTS.md` to the file you are
editing** — if you are editing `packages/records/src/MedicalRecords.tsx`, then
`packages/records/AGENTS.md` wins wherever the two disagree.
...
**Rspack 2.1 Module Federation**. One host, four remotes.
...
2. **Never set `lazyCompilation: true`** (or delete the explicit `lazyCompilation: false`).
   Rspack 2.1's CLI defaults `lazyCompilation.imports` to `true` for browser targets,
...
- **React Compiler is on** via `jsc.transform.reactCompiler: true` in `builtin:swc-loader`
  (the Rust port, new in Rspack 2.1). Auto-memoization happens at build time — **do not
